...
#include <xc.h>
#define _XTAL_FREQ 8000000      // Crystal FREQ
// CONFIG1
...
void USARTWriteChar(char ch) // USB comunication protocol
{
  while(!PIR1bits.TXIF);
...
unsigned char readEEPROM(unsigned char address)   //Read the EEPROM Memomry
{
...
void main(void) {
      OSCCONbits.IRCF = 0b111;              //Ports Initialitiation
 TRISB = 0x00;            //Ports Initialitiation
 TRISD = 0x00;            //Ports Initialitiation
 PORTD = 0x00;            //Ports Initialitiation
    PORTB = 0x00;           //Ports Initialitiation
     TRISA = 0x00;            //Ports Initialitiation
     PORTA = 0x00;            //Ports Initialitiation
     Test();
     USARTInit(9600);
      char pwd[] = {0x41,0x42,0x43,0x44,0x45,0x46};  // Array
      int cont = 0;   //Count
      int cont1 = 0;  //Count
     while(1)
     { 
         if(readEEPROM(0x8E) == 0xFF)  //Read Memomory
         {
             int va2 = 0;
             while(va2 == 0)
             {
         ...
         for(i ; i <= 5 ; i++)
         {
             ...
             __delay_ms(100);
             PORTAbits.RA1 = 0;
             if(pwd[i] == UART_Read()) //USB Read data
             {
                 va++;
             }
             else
             {
                 i = 5;
                 USARTWriteChar(0x40);  //USB write data
             }
         }
         if(va == 6)
         {
